...
static int lge_set_extcon_device(struct wcd_mbhc *mbhc, int status)
{
	...
	pr_debug("[LGE MBHC] edev state: %d, edev name: %s\n", result, mbhc->edev->name);
	return result;
}
#else
static int lge_set_extcon_device(struct wcd_mbhc *mbhc, int status)
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (is_pa_on) {
			wcd_mbhc_set_and_turnoff_hph_padac(mbhc);
			pr_debug("[LGE MBHC] %s: Impedance debugging patch PA [%d]\n", __func__, is_pa_on);
		}
#endif

#ifdef CONFIG_SND_SOC_HSDET_L_SWITCH
		if(es9218_hifi_exception_state_get() != ESS_PS_BYPASS) { // check ESS mode ESS_BYPASS = 2
			pr_debug("[LGE MBHC] %s: ess hifi exception status. To read the L/R impedance, change the ESS state from HIFI to Bypass", __func__);
			es9218_sabre_headphone_off();
			mdelay(1);
			es9218_sabre_headphone_on();
			mdelay(1);
		...
		if(mbhc->ess_hifi_exception) { // restore ess hifi mode
			pr_debug("[LGE MBHC] %s: restore ESS state. Bypass->HiFi", __func__);
			es9218_sabre_headphone_on();
			mdelay(1);
...
#ifdef CONFIG_MACH_LGE
		mbhc->LGE_HIGH_HPH_HEADSET = false;
		pr_debug("[LGE MBHC] %s: jack report for remove interrupt with none plug type %d\n", __func__,mbhc->current_plug);
		      ...
		      bool impedance_det_en)
{
	...
	if (ret) {
		dev_dbg("%s: missing %s in dt node\n", __func__, lge_extn_cable);
	}
#endif
	...
	pr_debug("%s register excon device\n",__func__);
	...
	pr_debug("%s register voc_mute_staus excon device\n",__func__);
#endif /* CONFIG_SND_LGE_VOC_MUTE_DET */
#ifdef CONFIG_SND_USE_MBHC_EXTN_CABLE
	mbhc->extn_cable = false;
	mbhc->extn_exception = false;
