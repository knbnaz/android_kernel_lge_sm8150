	...
	for (i = 0; i < rechecks; i++) {
		...
		if (!wait_for_completion_timeout(&display->esd_te_gate,
			...
			if (!display->panel->lge.panel_dead) {
				...
				DSI_DEBUG("Already in recovery state\n");
				goto out;
			}
		...
		int power_mode, void *disp)
{
	...
	if (power_mode == SDE_MODE_DPMS_OFF) {
		DSI_DEBUG("SDE_MODE_DPMS_OFF backlight 0\n");
		...
		if (blmap && blmap->map) {
			...
			if (rc) {
				DSI_ERR("failed to update backlight\n");
			}
		} else {
...
int dsi_display_cont_splash_config(void *dsi_display)
{
	...
	if (rc)
		DSI_ERR("lge_dsi_panel_drv_post_init failed, rc=%d\n", rc);
#endif
...
int dsi_display_splash_res_cleanup(struct  dsi_display *display)
{
	...
	DSI_INFO("Continuous splash is done\n");
#endif
	...
	if ((mode->dsi_mode_flags & DSI_MODE_FLAG_DMS) &&
		...
		if (display->panel->lge.use_ddic_reg_backup) {
			...
			if (display->panel->lge.ddic_reg_backup_complete) {
				...
				if (rc) {
					DSI_WARN("WARNING: fail to reinit backup,rc=%d\n", rc);
				}
			}
		}
	}
...
int dsi_display_post_kickoff(struct dsi_display *display)
{
	...
	if (c_conn->last_panel_power_mode == SDE_MODE_DPMS_ON) {
		...
		if ((display->panel->lge.bl_ex_lvl_unset >= 0)
			...
			if (rc) {
				DSI_ERR("failed to update backlight ex\n");
			}
		}
	}
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (is_skip_op_required(display)) {
		...
		if (is_need_register_backup(display->panel)) {
			...
			if (rc) {
				DSI_WARN("WARNING: fail to backup, rc=%d\n", rc);
			}
		}
		...
		if (lge_get_factory_boot() || lge_get_mfts_mode()) {
			if (display->panel->lge.use_panel_err_detect && !display->panel->lge.err_detect_irq_enabled) {
				if (display->panel->lge.ddic_ops->set_err_detect_mask && display->panel->lge.is_first_err_mask) {
					...
					mutex_unlock(&display->display_lock);
					if (rc) {
						DSI_ERR("enabling err detect is failed\n");
					} else {
