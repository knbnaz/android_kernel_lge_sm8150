...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	DP_DEBUG("DD Before Debounce Time : %s",	hpd ? "CONNECT" : "DISCONECT");
	if (lge_dp->led_status) {
		DP_DEBUG("skip DD HPD low operation for cover_led\n");
		return IRQ_HANDLED;
	}

	if (!hpd) {
		int state = atomic_read(&lge_dp->dd_5v_power_state);
		DP_DEBUG("hpd=low, 5v_state=%d, powere_mode=%d\n", state, is_dd_powermode());
...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	if (gpio_hpd->hpd && hpd) {
		DP_DISPLAY("Skip duplicated hpd event during debounce!\n");
		return IRQ_HANDLED;
	}
#endif
...
static void dp_gpio_hpd_work(struct work_struct *work)
{
	...
	DP_DEBUG("%s : DD GPIO_HPD : %s", __func__,
	...
	if (!gpio_hpd->hpd) {
		...
		ss_phy = devm_usb_get_phy_by_phandle(gpio_hpd->dev,
				"usb-phy", 1);
		if (IS_ERR(ss_phy)) {
			DP_ERR("unable to get ssphy device on DD\n");
			ret = PTR_ERR(ss_phy);
			if (ret) {
				DP_DEBUG("setting pm-qos-latency to zero on DD.\n");
			}
		} else {
