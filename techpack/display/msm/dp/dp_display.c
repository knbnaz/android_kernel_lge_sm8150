...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY) || IS_ENABLED(CONFIG_LGE_DUAL_SCREEN)
static void lge_dp_display_send_hpd_event(struct dp_display_private *dp, bool hpd)
{
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	if (get_lge_cover_ops()) {
		...
		if (ops->get_recovery_state() == RECOVERY_LTFAIL_BEGIN ||
			...
			DP_DEBUG("recovery for ltfail\n");
			goto skip_event;
		}
	}

	DP_DEBUG("hpd_high=%d, dd_hpd_high=%d on %pS\n", dp->hpd->hpd_high, dp->dd_hpd->hpd_high, __builtin_return_address(0));
...
skip_event:
#elif IS_ENABLED(CONFIG_LGE_DUAL_SCREEN)
	{
		...
		DP_DEBUG("hpd = %d, is_ds2_connected = %d get_cover_display_state = %d skip_uevent =%d\n",
...
#if IS_ENABLED(CONFIG_LGE_COVER_DISPLAY)
	if (dp->dd_hpd->hpd_high) {
		DP_DEBUG("%s : default Orientation is CC1 for DD", __func__);
...
static int dp_display_handle_disconnect(struct dp_display_private *dp)
{
	...
	if (COVER_DISPLAY_STATE_CONNECTED_POWERDROP == get_cover_display_state()) {
		...
		int count = 0;
		DP_DEBUG("waiting for finising cover display recovery\n");
		...
		if (COVER_DISPLAY_STATE_CONNECTED_ON == get_cover_display_state()) {
			DP_DEBUG("abort as cover display recovered\n");
			return 0;
		} else {
			DP_DEBUG("turn DD power off\n");
	...
	if (dp->dp_display.lge_dp.skip_uevent && dp->power_on && !dp->hpd->hpd_high) {
		int retry_count = 500;
		DP_DEBUG("%s : waiting for powermode malfunction\n", __func__);
		...
		DP_DEBUG("%s : powermode waiting finished\n", __func__);
	}
#endif
...
static int dp_display_usbpd_attention_cb(struct device *dev)
{
	...
	if (!is_ds2_connected()) {
		dp->dp_display.lge_dp.skip_uevent = 0;
		DP_DEBUG("[drm-dp] USBDP connection, skip_uevent init to %d\n", dp->dp_display.lge_dp.skip_uevent);
	}
#endif
...
static int dp_init_dd(struct dp_display_private *dp)
{
	...
	if (IS_ERR(dp->dd_hpd)) {
		...
		DP_ERR("failed to initialize DD hpd, rc = %d\n", rc);
		dp->dd_hpd = NULL;
		goto error_dd_hpd;
	}
...
#ifdef CONFIG_LGE_COVER_DISPLAY
		set_cover_display_state(COVER_DISPLAY_STATE_CONNECTED_OFF);
		DP_DEBUG("%s : send disconnect callback for twisted powermode \n", __func__);
#endif
		dp_display_state_show("[not ready]");
		goto end;
	}
...
static int dp_display_probe(struct platform_device *pdev)
{
	...
	if (!dp->ws) {
		DP_ERR("faile to resister wakeup source\n");
	}
#endif
...
bool is_dd_connected(void)
{
	...
	if (IS_ERR_OR_NULL(dp) || IS_ERR_OR_NULL(dp->dd_hpd)) {
		DP_ERR("DD hpd is not initialized yet\n");
		ret = false;
	} else {
