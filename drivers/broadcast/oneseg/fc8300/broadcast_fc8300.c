...
#define LGE_FC8300_DRV_VER  "2.00.00"
...
//change wakeup_source_init() -> wakeup_source_register() by SM8350 (kernel 5.4)
    tpdata->wakelock = wakeup_source_register(&tpdata->spidev->dev, dev_name(&client->dev));
    if(!tpdata->wakelock) {
...
//change wakeup_source_trash() -> wakeup_source_unregister() by SM8350 (kernel 5.4)
    wakeup_source_unregister(fc8300_pdata.wakelock);
