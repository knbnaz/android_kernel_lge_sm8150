...
#include "drv/inc/fci_tun.h"
#include "drv/inc/fc8180_regs.h"
#include "drv/inc/fc8180_isr.h"
#include "drv/inc/fci_hal.h"
...
int    broadcast_fc8180_drv_start(void)
{
    int rc;
    rc = OK;
    print_log(NULL, "[dtv] broadcast_drv_start\n");
...
int    broadcast_fc8180_get_stop_mode(void)
{
    int rc;
    rc = OK;
    print_log(NULL, "[dtv] broadcast_get_stop_mode\n");
...
int    broadcast_fc8180_drv_if_power_on(void)
{
    ...
    print_log(NULL, "[dtv] broadcast_drv_if_power_on\n");
    if (!isdbt_fc8180_is_power_on())
...
{
    int rc = OK;
    print_log(NULL, "[dtv] broadcast_drv_if_power_off\n");
    if (isdbt_fc8180_is_power_on()) {
        rc = isdbt_fc8180_power_off();
    } else {
        print_log(NULL, "[dtv] warning-already power off\n");
    }
...
int    broadcast_fc8180_drv_if_open(void)
{
    ...
    print_log(NULL, "[dtv] broadcast_fc8180_drv_if_open\n");
...
int    broadcast_fc8180_drv_if_set_channel(struct broadcast_dmb_set_ch_info *udata)
{
    ...
    if(OnAir == 0 || udata == NULL) {
        print_log(NULL, "[dtv] broadcast_drv_if_set_channel error [!OnAir]\n");
    ...
    if(udata->channel<14 || udata->channel>69) {
        print_log(NULL, "[dtv] channel information error\n");
    ...
    print_log(NULL, "[dtv][fc8180] channel : %d, %d, %d, scan OK\n", udata->channel, udata->mode, udata->subchannel);
...
int    broadcast_fc8180_drv_if_detect_sync(struct broadcast_dmb_sync_info *udata)
{
    int sync;
    if(OnAir == 0) {
        print_log(NULL, "[dtv] broadcast_drv_if_detect_sync error [!OnAir]\n");
        return ERROR;
    }
...
static void broadcast_fc8180_drv_if_get_oneseg_sig_info(struct fc8180Status_t *pst, struct broadcast_dmb_control_info *pInfo, s32 brd_type)
{
    int ber;
    int per;
    int cn;
    ...
    print_log(NULL, "[dtv][monitor] lock[%d] Antenna[%d] cn[%d] ber[%d] per[%d] rssi[%d] errTSP[%d/%d]\n",
...
int    broadcast_fc8180_drv_if_get_sig_info(struct broadcast_dmb_control_info *pInfo)
{
    ...
    case ENUM_GET_CN:
        ...
        print_log(NULL, "[dtv][MMBi][monitor] cn[%d]\n", pInfo->sig_info.info.mmb_info.cn);
    ...
    case ENUM_GET_RECEIVE_STATUS:
        ...
        print_log(NULL, "[dtv][MMBi][monitor] rcv status[%d]\n", pInfo->sig_info.info.mmb_info.receive_status);
        /* for debugging log */
    ...
    case ENUM_GET_RSSI:
        ...
        print_log(NULL, "[dtv][MMBi][monitor] rssi[%d]\n", pInfo->sig_info.info.mmb_info.rssi);
    ...
    case ENUM_GET_SCAN_STATUS:
        ...
        print_log(NULL, "[dtv][MMBi][monitor] scan status[%d]\n", pInfo->sig_info.info.mmb_info.scan_status);
        /* for debugging log */
    ...
    case ENUM_GET_SYS_INFO:
        ...
        print_log(NULL, "[dtv][MMBi][monitor] sys info[%d]\n", pInfo->sig_info.info.mmb_info.sysinfo);
        /* for debugging log */
    ...
    case ENUM_GET_TMCC_INFO:
        ...
        print_log(NULL, "[dtv][MMBi][monitor] tmcc info[%d]\n", pInfo->sig_info.info.mmb_info.tmccinfo);
        /* for debugging log */
    ...
    default:
        print_log(NULL, "[dtv][MMBi][monitor] sig_info unknown command[%d]\n", pInfo->cmd_info.cmd);
...
{
    if (OnAir == 0) {
        print_log(NULL, "[dtv][no semaphore] broadcast_drv_if_get_ch_info error [!OnAir]\n");
...
int    broadcast_fc8180_drv_if_get_dmb_data(struct broadcast_dmb_data_info *pdmb_data)
{
    ...
    if(pdmb_data->data_buf_addr == 0) {
        print_log(NULL, "[dtv] broadcast_drv_if_get_dmb_data[ERR] data_buf is null\n");

        return ERROR;
    }

    if(pdmb_data->data_buf_size < 188) {
        print_log(NULL, "[dtv] broadcast_drv_if_get_dmb_data[ERR] buffsize < 188\n");

        return ERROR;
    }

    total_len = isdbt_fc8180_get_ts((void*)((unsigned long)pdmb_data->data_buf_addr), pdmb_data->data_buf_size);
...
int    broadcast_fc8180_drv_if_reset_ch(void)
{
    ...
    if(OnAir == 0) {
        print_log(NULL, "[dtv] broadcast_drv_if_reset_ch error [!OnAir]\n");

        return ERROR;
    }

    print_log(NULL, "[dtv] broadcast_drv_if_reset_ch\n");
...
int    broadcast_fc8180_drv_if_user_stop(int mode)
{
    ...
    if(OnAir == 0) {
        print_log(NULL, "[dtv][no semaphore] broadcast_drv_if_user_stop error [!OnAir]\n");

        return ERROR;
    }

    print_log(NULL, "[dtv][no semaphore] broadcast_drv_if_user_stop\n");
...
int    broadcast_fc8180_drv_if_select_antenna(unsigned int sel)
{
    ...
    if(OnAir == 0) {
        print_log(NULL, "[dtv][no semaphore] broadcast_drv_if_select_antenna error [!OnAir]\n");
...
int    broadcast_fc8180_drv_if_isr(void)
{
    ...
    if(OnAir == 0) {
        print_log(NULL, "[dtv] broadcast_drv_if_isr error [!OnAir]\n");
