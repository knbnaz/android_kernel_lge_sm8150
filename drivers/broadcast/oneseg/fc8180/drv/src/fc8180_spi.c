    ...
    , u8 *txbuf, u16 tx_length, u8 *rxbuf, u16 rx_length)
{
    ...
    if((tx_length + rx_length) > 16) {
        ...
        t[0].rx_buf = &rdata_buf[0];
        t[0].len = (tx_length + rx_length) - ((tx_length + rx_length) % 16);
        t[0].cs_change = 0;
        t[0].bits_per_word = 8;
        t[1].tx_buf = &wdata_buf[0];
        t[1].rx_buf = &rdata_buf[t[0].len];
        t[1].len = (tx_length + rx_length) % 16;
        t[1].cs_change = 0;
        t[1].bits_per_word = 8;

        res = spi_sync(spi, &message);

        memcpy(rxbuf, t[0].rx_buf + tx_length, t[0].len - tx_length);
        memcpy(rxbuf + (t[0].len - tx_length), t[1].rx_buf, t[1].len);
    } else
#endif
    {
        memset(&x, 0, sizeof x);
...
s32 fc8180_spi_init(HANDLE handle, u16 param1, u16 param2)
{
    ...
    fc8180_spi = FCI_GET_SPI_DRIVER();
        if(fc8180_spi == NULL) {
            print_log(0, "fc8180_spi device is not ready \n");
            return BBM_NOK;
        } else {
