...
static int s3706_lpwg_fail_control(struct device *dev, u16 value)
{
	...
	ret_page = s3706_set_page(dev, LPWG_PAGE);
	if (ret_page < 0) {
		TOUCH_E("failed to set LPWG_PAGE (ret: %d)\n", ret_page);
		return ret_page;
	}
...
static int s3706_tci_knock(struct device *dev)
{
	...
	for (i = 0; i < 2; i++) {
		if ((ts->tci.mode & (1 << i)) == 0x0) {
			lpwg_data[0] = 0;
			ret = s3706_write(dev, tci_reg[i], lpwg_data, sizeof(u8));
			if (ret < 0) {
				TOUCH_E("failed to tci_reg write (ret: %d)\n", ret);
				goto error;
			}
		} else {
			info = &ts->tci.info[i];
			...
			ret = s3706_write(dev, tci_reg[i], lpwg_data, sizeof(lpwg_data));
			if (ret < 0) {
				TOUCH_E("failed to tci_reg write (ret: %d)\n", ret);
				goto error;
			}
	...
	if ((ts->tci.mode & (1 << i)) == 0x0) {
		lpwg_data[0] = 0;
		ret = s3706_write(dev, LPWG_TAPCOUNT_REG, lpwg_data, sizeof(u8));
		if (ret < 0) {
			TOUCH_E("failed to LPWG_TAPCOUNT_REG write (ret: %d)\n", ret);
			goto error;
		}
	} else {
		info = &ts->tci.info[i];
		...
		lpwg_data[4] = info->tap_distance;
		lpwg_data[6] = (info->intr_delay << 1 | 1);
		ret = s3706_write(dev, LPWG_TAPCOUNT_REG, lpwg_data, sizeof(lpwg_data));
		if (ret < 0) {
			TOUCH_E("failed to LPWG_TAPCOUNT_REG write (ret: %d)\n", ret);
			goto error;
		}
