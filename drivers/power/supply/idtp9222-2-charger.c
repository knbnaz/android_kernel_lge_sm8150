...
static bool idtp9222_set_onpad(struct idtp9222_struct *idtp9222, bool onpad) {
	...
	if (idtp9222->status_onpad == onpad && !idtp9222->force_online) {
...
static bool idtp9222_set_vrect(struct idtp9222_struct *idtp9222) {
	...
	if (12000 > ((idtp9222->configure_maxvolt) *
	             (idtp9222->configure_maxcurr) / 1000))
		max_pwr = POWER_13W;
	...
	if (idtp9222->opmode_midpower) {
		value = (u16)((((request_volt + CONF_MARGIN_VOL) / 21) << 12) / 1000);
...
static bool psy_set_disable(struct idtp9222_struct *idtp9222, bool disable) {
	if (idtp9222->wlc_disable)
		vote(idtp9222->wlc_disable, DISABLE_BY_WA, disable, 0);
...
static int psy_property_set(struct power_supply *psy,
	...
	case POWER_SUPPLY_PROP_EXT_INPUT_SUSPEND:
		psy_set_disable(idtp9222, !!val->intval);
		break;
