#define pr_fmt(fmt) "CHG: [CEIL] %s: " fmt, __func__
...
bool charging_ceiling_vote(enum charging_supplier charger)
{
	...
	if (ceiling_me.charger_type == CHARGING_SUPPLY_USB_2P0) {
		veneer_voter_passover(VOTER_TYPE_IUSB, 900, ceiling_me.charger_sdpmax);
	} else if (charger_fabcable(ceiling_me.charger_type)) {
		veneer_voter_passover(VOTER_TYPE_IUSB, ceiling_value.iusb, true);
	} else {
		veneer_voter_passover(VOTER_TYPE_IUSB, UNCEILING, false);
	}
...
{
	bool success = veneer_voter_register(&ceiling_me.voter_iusb, VOTER, VOTER_TYPE_IUSB, false)
	...
	if (!(dnode && of_device_is_available(dnode))) {
		pr_ceil("Ceiling disabled\n");
		return true;
	}
	ceiling_me.enable = true;
