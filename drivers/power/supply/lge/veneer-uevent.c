#define pr_fmt(fmt) "CHG: [UEVENT] %s: " fmt, __func__
...
static struct filter {
	...
	{ "POWER_SUPPLY_CAPACITY_RAW",		255,		25 }, // batt       / msoc (0~255)
	{ "POWER_SUPPLY_VOLTAGE_OCV",		50000,		24 }, // batt       / OCV(uv)
	{ "POWER_SUPPLY_CHARGE_COUNTER",	10000,		27 }, // batt,      / Charging counter(uAh)
	{ "POWER_SUPPLY_VOLTAGE_NOW",		50000,		24 }, // batt, usb  / batt, input volt(uv)
	{ "POWER_SUPPLY_CURRENT_NOW",		200000,		24 }, // batt,      / current(ua)
	{ "POWER_SUPPLY_TEMP",				10,			17 }, // batt       / temp(x)
	{ "POWER_SUPPLY_CHARGER_TEMP",		10,			25 }, // batt       / temp(x)
	{ "POWER_SUPPLY_POWER_AVG",			10000000,	22 }, // batt       / temp(x)
	{ "POWER_SUPPLY_POWER_NOW",			10000000,	22 }, // batt       / temp(x)
};
