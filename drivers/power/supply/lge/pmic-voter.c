 ...
 */
static bool vote_set_any(struct votable *votable, int client_id,
 ...
 *	Must be called with the votable->lock held
 */
static bool vote_min(struct votable *votable, int client_id,
				int *eff_res, int *eff_id)
{
	int i;
	int now_pri = get_effective_priority_locked(votable);
	int eff_pri = -EINVAL;
	...
	*eff_id = -EINVAL;
	for (i = 0; i < votable->num_clients && votable->client_strs[i]; i++) {
		if (votable->votes[i].enabled && 
				((*eff_res > votable->votes[i].value && eff_pri == votable->votes[i].priority) ||
				eff_pri < votable->votes[i].priority)) {
 ...
 *	Must be called with the votable->lock held
 */
static bool vote_max(struct votable *votable, int client_id,
				int *eff_res, int *eff_id)
{
	int i;
	int now_pri = get_effective_priority_locked(votable);
...
{
	if (!votable || votable->effective_client_id < 0)
		return PRIORITY_LOW;

	if (votable->force_active)
		return PRIORITY_MAX;

	return votable->votes[votable->effective_client_id].priority;
}
...
int get_effective_priority(struct votable *votable)
{
	...
	lock_votable(votable);
	priority = get_effective_priority_locked(votable);
	unlock_votable(votable);
	return priority;
}
...
{
	return vote_priority(votable, client_str, enabled, val, PRIORITY_LOW);
}

int vote_priority(struct votable *votable, const char *client_str, bool enabled, int val, int priority)
{
	...
	if ((votable->votes[client_id].enabled == enabled) &&
		...
		(votable->votes[client_id].priority == priority)) {
		pr_debug("%s: %s,%d same vote %s of val=%d pri = %d\n",
				votable->name,
	...
	switch (votable->type) {
	case VOTE_MIN:
		pri_changed = vote_min(votable, client_id,
				&effective_result, &effective_id);
		break;
	case VOTE_MAX:
		pri_changed = vote_max(votable, client_id,
				&effective_result, &effective_id);
		break;
	case VOTE_SET_ANY:
		pri_changed = vote_set_any(votable, client_id,
				&effective_result, &effective_id);
		break;
	default:
		goto out;
	 ...
	 */
	if (!votable->voted_on || pri_changed
...
out:
	...
	if (found)
		...
		return create_votable(name, VOTE_DUMMY, NULL, NULL);
}
...
#define RECREATE_VOTE	"RECREATE"
#define INIT_VALUE	1
struct votable *recreate_votable(struct votable *votable,
				int votable_type,
				int (*callback)(struct votable *votable,
					void *data,
					int effective_result,
				...
				void *data)
{
	if (votable->type == VOTE_DUMMY) {
		votable->callback = callback;
		votable->type = votable_type;
		votable->data = data;
		vote(votable, RECREATE_VOTE, false, INIT_VALUE);
	} else
				...
				void *data)
{
	...
	if (votable_type != VOTE_DUMMY) {
		votable = find_votable(name);
		if (votable)
			return recreate_votable(votable, votable_type, callback, data);
	}
