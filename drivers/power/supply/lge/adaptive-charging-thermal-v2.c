...
#define pr_fmt(fmt) "CHG: [ACTM] %s: " fmt, __func__
...
static bool is_lcdon(void)
{
    ...
    if (get_veneer_param(VENEER_FEED_LCDON_STATUS, &lcdon)) {
        pr_actm(ERROR, "get_veneer_param(lcdon status) failure, error\n");
        lcdon = 0;
    }
...
static int get_actm_mode(void)
{
    ...
    if (mode == ACTM_MODE_AUTO) {
        get_veneer_param(VENEER_FEED_CAPACITY, &msoc);
...
static int get_dt_temp(struct _actm_dt *dt, int stage)
{
    ...
    if (get_veneer_param(VENEER_FEED_ACTM_LCDON_TEMP_OFFSET, &lcdon_offset)) {
        pr_actm(ERROR, "get_veneer_params(lcdon temp offset) failure, error\n");
        lcdon_offset = 0;
    }

    actm_me.lcdon_temp_offset = 0;
    if (is_lcdon())
        ...
        get_actm_mode() > ACTM_MODE_DISABLE)
    {
        ...
        pr_actm(EVALUATE,
            "%s: return=%d (criteria=%d, offset=%d, lcdon=%d, w-to-n=%d)\n",
...
static void set_actm_current(int set_curr, bool sidecar)
{
    ...
    if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_9W ||
        ...
        if (actm_me.max_volt > 0 &&
            actm_me.conn == CHARGING_SUPPLY_WIRELESS_15W &&
            /* minimize 12V area for mis-align */
            set_curr > (actm_me.epp_volt * actm_me.epp_curr / 1000))
    ...
    if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_5W
        ...
        set_veneer_param(VENEER_FEED_VDC, vdc);
        if (sidecar)
...
static int find_current_step(struct _actm_dt *dt, int pre, int now)
{
    ...
    if (actm_me.cp_type == CP_PPS) {
        if (!get_veneer_param(VENEER_FEED_FCC, &now_fcc)) {
            if (now_fcc > ACTM_MIN_INTERVAL_FCC)
                curr_step = curr_step * 2;
                curr_step = min(curr_step, curr_step_array[2]);
        }
    }
...
static int actm_act(struct _actm_dt *dt, int policy, int temp)
{
    ...
    int now_min_curr = 0, idc = 0, vdc = 0, now_curr = 0, set_curr = 0;
    ...
    if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_5W
        ...
        if (get_veneer_param(VENEER_FEED_IDC, &idc)) {
            pr_actm(ERROR,
                "get_veneer_param(id=%d) failure, error\n", VENEER_FEED_IDC);
            return -1;
        }

        if (get_veneer_param(VENEER_FEED_VDC, &vdc)) {
            pr_actm(ERROR,
                "get_veneer_param(id=%d) failure, error\n", VENEER_FEED_VDC);
            return -1;
        }
        ...
        if (get_veneer_param(VENEER_FEED_FCC, &now_curr)) {
            pr_actm(ERROR,
                "get_veneer_param(id=%d) failure, error\n", VENEER_FEED_FCC);
            return -1;
        }
...
static bool actm_is_sidecar_triggered(void)
{
    ...
    if (actm_me.dt[conn_type].sidecar[actm_mode].is_on[lcdon])
        return true;
...
static int actm_job(struct _actm_dt *dt, int temp)
{
    ...
    if (now_stage < ACTM_STAGE_NORMAL) {
        if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_15W)
            set_veneer_param(VENEER_FEED_VDC, actm_me.max_volt);
        else if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_9W)
            set_veneer_param(VENEER_FEED_VDC, actm_me.epp_volt);
        else if (actm_me.conn == CHARGING_SUPPLY_WIRELESS_5W)
            set_veneer_param(VENEER_FEED_VDC, actm_me.bpp_volt);

        if (dt->max_power_15w[get_actm_mode()] > 0 &&
            actm_me.conn == CHARGING_SUPPLY_WIRELESS_15W) {
...
static void update_actm_param(int conn)
{
    ...
    if (conn == WIRED) {
        ...
        if (!actm_me.enable_cp_charging)
            ...
            if (actm_me.conn == CHARGING_SUPPLY_USB_PD_PPS)
                actm_me.cp_type = CP_PPS;
            else if (actm_me.conn == CHARGING_SUPPLY_DCP_QC3)
...
static void actm_status_work(struct work_struct* work)
{
    ...
    const char str_charger[CHARGING_SUPPLY_MAX][13] = {
	    ...
	    "USB2", "USB3", "CDP", "USB_PD", "PD_PPS",
	    "FACTORY_56K", "FACTORY_130K", "FACTORY_910K",
	    "WIRELESS_5W", "WIRELESS_9W", "WIRELESS_15W"
    };

    if (get_veneer_param(VENEER_FEED_ACTM_MODE, &mode)) {
        pr_actm(ERROR, "get_veneer_param(actm mode) failure, error\n");
        actm_init();
        return;
    }
    set_actm_mode(mode);
    ...
    set_veneer_param(VENEER_FEED_ACTM_MODE_NOW, actm_me.mode);
    ...
    if (get_veneer_param(VENEER_FEED_STATUS_RAW, &status_buf)) {
        pr_actm(ERROR, "get_veneer_param(status) failure, error\n");
        actm_init();
        return;
    }
    ...
    if (status_buf != POWER_SUPPLY_STATUS_CHARGING) {
        get_veneer_param(VENEER_FEED_USB_PRESENT, &usb_present);
        get_veneer_param(VENEER_FEED_DC_PRESENT, &wireless_present);
    ...
    if (get_veneer_param(VENEER_FEED_CHARGER_TYPE, &conn_buf)) {
        pr_actm(ERROR, "get_veneer_param(charger type) failure, error\n");
        actm_init();
        return;
    }

    if (get_veneer_param(VENEER_FEED_SENSOR_BATT, &temp_buf[BATT])) {
        pr_actm(ERROR, "get_veneer_param(batt) failure, error\n");
        actm_init();
        return;
    }

    if (get_veneer_param(VENEER_FEED_SENSOR_VTS, &temp_buf[VTS])) {
        pr_actm(ERROR, "get_veneer_param(vts) failure, error\n");
        actm_init();
        return;
    }
    ...
    if (!actm_job(&actm_me.dt[conn], max(temp[BATT], temp[VTS]))) {
        ...
        if (conn == WIRED) {
            get_veneer_param(VENEER_FEED_FCC, &now_fcc);
            if (now_fcc >= ACTM_MIN_INTERVAL_FCC)
...
static bool actm_parse_dt(struct device_node* dnode)
{
    ...
    if (d_idtp9222) {
        ...
        actm_me.max_volt = 0;
        actm_me.max_curr = 0;
        pr_actm(ERROR, "idtp9222-charger dtsi error -> set default(max=0V, epp=9V, bpp=6V)\n");
    }
...
bool actm_create(   struct device_node* dnode)
{
	...
	if (!(dnode && of_device_is_available(dnode))) {
		pr_actm(ERROR, "actm_create disabled.\n");
		return true;
	}
