#define pr_fmt(fmt) "CHG: [BOOTMOD] %s: " fmt, __func__
...
void unified_bootmode_cable(const char* arg)
{
	strncpy(bootmode_cable, arg, LENGTH_STRING_BUFFER-1);
	unified_bootmode_setup();
}

void unified_bootmode_android(const char* arg)
{
...
const char* unified_bootmode_operator(void)
{
	...
	case OPERATOR_DCM  :	return "DCM";
	case OPERATOR_SB   :	return "SB";
	case OPERATOR_KDDI :	return "KDDI";
...
const char* unified_bootmode_region(void)
{
	...
	case REGION_CAN :	return "CA";
	case REGION_JPN :	return "JP";
	case REGION_KOR :	return "KR";
	case REGION_USA :	return "US";
...
void unified_bootmode_get_property_operator(const char* s, char* property)
{
	...
	strncpy(property, s, MAX_PROPERTY_LENGTH);
	strncat(property, unified_bootmode_operator(), MAX_PROPERTY_LENGTH);
...
void unified_bootmode_get_property_region(const char* s, char* property)
{
	...
	// Building property name
	strncpy(property, s, MAX_PROPERTY_LENGTH);
	strncat(property, unified_bootmode_region(), MAX_PROPERTY_LENGTH);
	for (i = 0; i < strlen(property); ++i)
		property[i] = tolower(property[i]);
}
...
static int __init setup_bootmode_plmn(char* s)
{
	if (!strcmp(s, "KT"))
		...
		bootmode_operator = OPERATOR_DCM;
	else if (!strcmp(s, "SB"))
		bootmode_operator = OPERATOR_SB;
	else if (!strcmp(s, "KDDI"))
		bootmode_operator = OPERATOR_KDDI;
	else
		bootmode_operator = OPERATOR_MISC;
	...
	else if (!strcmp(s, "DCM") || !strcmp(s, "SB") || !strcmp(s, "KDDI"))
