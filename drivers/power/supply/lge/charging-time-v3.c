...
static struct lgttf {
	...
	struct delayed_work bcc_ttf_work;

	enum charging_supplier type;
	int bsm_ttf;
	int bsm_enable;
	bool enable;
} lgttf = {0, };
...
{
	const int ttf_zero_index_margin = 1;
	int rc = 0, i = 0;
	...
	for (i = (LGTTF_SLOT_COUNT-1); i >= 0 ; i--) {
		if (i > (lgttf.base.full_soc - ttf_zero_index_margin))
			table->data[i].ttf = 0;
		else
			table->data[i].ttf =
...
int charging_time_remains(void)
{
	...
	int rawsoc = -1, ui_soc = -1;

	if(!lgttf.enable)
		return -1;

	if (get_veneer_param(VENEER_FEED_LG_UI_SOC_255, &rawsoc))
		return -1;
	...
	if (ui_soc >= 100)
		rawsoc = FULL_CAPACITY_RAW;
	if (ui_soc < 100)
		rawsoc = min(rawsoc, FULL_CAPACITY_RAW - 1);

	table = &lgttf.table[actm][selected];
	if (!table)
		return -1;

	result_new = table->data[rawsoc].ttf + lgttf.rt.bcc_ttf;
	if (result_new != result_old) {
		pr_chgtime(UPDATE,
			"report...soc=%d(%d, %d), ttf=%d+%d=%dsec(%dmin) "
			"(id=%s, actm=%s, type=%s, power=%d)\n",
			rawsoc, table->data[rawsoc].ui_soc, table->data[rawsoc].raw_soc,
			table->data[rawsoc].ttf, lgttf.rt.bcc_ttf,
			...
			(actm < LGTTF_ACTM_THERMAL || actm > LGTTF_ACTM_MAX) ?
				"ERROR" : str_actm[actm],
			(type < LGTTF_TYPE_CP_PPS || type > LGTTF_TYPE_WIRELESS) ?
				"ERROR" : str_chg_type[type],
			lgttf.rt.power);
	}

	if (result_new > 0 && result_new < 60)
		result_new = 60;
...
static void lgttf_work_func(struct work_struct *work)
{
	...
	int power = 0, bcc_ttf = 0, bcc_enable = 0, charger = 0;

	if (get_veneer_param(VENEER_FEED_CHARGER_TYPE, &charger)) {
	...
	if (charger == CHARGING_SUPPLY_WIRELESS_5W ) {
		...
		lgttf.rt.chg_type = LGTTF_TYPE_WIRELESS;
	} else if (charger == CHARGING_SUPPLY_USB_PD_PPS) {
		lgttf.rt.chg_type = LGTTF_TYPE_CP_PPS;
		lgttf.rt.selected = LGTTF_ID_CP_PPS;
	} else if (charger == CHARGING_SUPPLY_DCP_QC3) {
	...
	get_veneer_param(VENEER_FEED_POWER_NOW, &power);
	get_veneer_param(VENEER_FEED_BSM_TTF, &bcc_ttf);
	get_veneer_param(VENEER_FEED_BSM_ENABLE, &bcc_enable);
	...
	if (lgttf.rt.bcc_ttf != bcc_ttf) {
		lgttf.rt.bcc_ttf = bcc_ttf;
		pr_chgtime(UPDATE, "BCC ttf changed : %d", lgttf.rt.bcc_ttf);
	}

	if (lgttf.rt.bcc_enable != bcc_enable) {
		lgttf.rt.bcc_enable = bcc_enable;
		pr_chgtime(UPDATE, "BCC enable changed : %d", lgttf.rt.bcc_enable);

		if (lgttf.rt.bcc_enable) {
			veneer_voter_set(&lgttf.ibat_voter, VOTE_TOTALLY_BLOCKED);
			schedule_delayed_work(&lgttf.bcc_ttf_work, msecs_to_jiffies(0));
		} else {
			if (delayed_work_pending(&lgttf.bcc_ttf_work)) {
				pr_chgtime(UPDATE, "Cancel pending bcc ttf work!");
				cancel_delayed_work(&lgttf.bcc_ttf_work);
			}
...
#define BCC_TTF_UNIT	60
static void bcc_ttf_work(struct work_struct *work)
{
	pr_chgtime(UPDATE, "BCC TTF add %ds", lgttf.rt.bcc_ttf);
	lgttf.rt.bcc_ttf -= BCC_TTF_UNIT;

	if (lgttf.rt.bcc_ttf <= 0) {
		lgttf.rt.bcc_ttf = 0;
		veneer_voter_release(&lgttf.ibat_voter);
	} else {
		schedule_delayed_work(&lgttf.bcc_ttf_work, msecs_to_jiffies(BCC_TTF_UNIT * 1000));
	}
}
...
{
	int charger = 0, bsm_ttf = 0, bsm_enable = 0;
	...
	if (get_veneer_param(VENEER_FEED_CHARGER_TYPE, &charger)
			|| get_veneer_param(VENEER_FEED_BSM_TTF, &bsm_ttf)
			|| get_veneer_param(VENEER_FEED_BSM_ENABLE, &bsm_enable))
		return ret;

	if (lgttf.type != charger
			|| lgttf.bsm_ttf != bsm_ttf
			|| lgttf.bsm_enable != bsm_enable) {
...
{
	int fullraw = FULL_CAPACITY_RAW;  /* fullraw is always 255 as to ui soc */

	if (!(dnode && of_device_is_available(dnode))) {
		pr_chgtime(ERROR, "charging_time disabled.\n");
		return true;
	}
	lgttf.enable = true;
	...
	INIT_DELAYED_WORK(&lgttf.bcc_ttf_work, bcc_ttf_work);
