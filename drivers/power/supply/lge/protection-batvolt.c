#define pr_fmt(fmt) "CHG: [BVP] %s: " fmt, __func__
...
static bool set_multi_fv(int mode)
{
	...
	rc = get_veneer_param(VENEER_FEED_CHARGER_TYPE, &charger);
	...
	rc = get_veneer_param(VENEER_FEED_BATT_PROFILE_FV_VOTER, &batt_profile_fv);
	if (!rc && batt_profile_fv > 0) {
		if (batt_profile_fv != bvp_me.multi_fv_mvolt[mode_local]) {
			rc = set_veneer_param(VENEER_FEED_BATT_PROFILE_FV_VOTER,
...
static void protection_batvolt_work(struct work_struct *work)
{
	...
	if (!get_veneer_param(VENEER_FEED_BAT_VOLTAGE_NOW, &vbat_now)
			&& !get_veneer_param(VENEER_FEED_FCC, &icap_now)
			&& !get_veneer_param(VENEER_FEED_BATT_PSY_IBAT_NOW, &ibat_now)
			&& !get_veneer_param(VENEER_FEED_CHARGE_TYPE, &chg_now)) {
		if (irc_enabled
...
//FIXED_ME
		if (chg_now != POWER_SUPPLY_CHARGE_TYPE_STANDARD
			&& icap_now <=
...
//FIXED_ME
	...
	/*Need to update :add checking charging_step*/
	if (ui_soc >= FULL_SOC)
		set_multi_fv(MFV_CV);
	else
		set_multi_fv(MFV_CC);
