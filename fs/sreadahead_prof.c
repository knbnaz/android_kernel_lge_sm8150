...
#ifdef CONFIG_APPLICATION_MANAGER
char prelaunch_application_cmdline[256];

static ssize_t prelaunch_application_read(struct file *filp,
	struct kobject *kobj, struct bin_attribute *attr, char *buff,
	loff_t off, size_t count)
{
	if (0 < off)
		return 0;

	memcpy(buff, prelaunch_application_cmdline, strlen(prelaunch_application_cmdline) + 1);

	return strlen(prelaunch_application_cmdline);
}
#endif
...
#ifdef CONFIG_APPLICATION_MANAGER
static struct bin_attribute prelaunch_application_data =
	__BIN_ATTR(prelaunch_application, S_IRUGO, prelaunch_application_read, NULL, 0);
#endif
...
static int is_system_ext_partition(unsigned char *fn) {
	return strncmp((const char*)fn, "/system_ext/", 12) == 0 ? 1 : 0;
}
...
static int sreadahead_prof_RUN(struct file *filp, size_t len, loff_t pos)
{
	...
	strncpy(data.name, data.name + 20, buflen); /* remove "/first_stage_ramdisk" from data.name */

	if (is_system_partition(data.name) == 0 && is_system_ext_partition(data.name) == 0 &&
